    ...
    <section className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="rounded-xl border bg-white p-4 dark:bg-zinc-900">
          <p className="text-sm text-zinc-500">Total Articles</p>
        ...
        <div className="rounded-xl border bg-white p-4 dark:bg-zinc-900">
          <p className="text-sm text-zinc-500">Trending Posts</p>
          <p className="text-2xl font-bold">{metrics.trendingPosts.length}</p>
        </div>
        <div className="rounded-xl border bg-white p-4 dark:bg-zinc-900">
          <p className="text-sm text-zinc-500">Ad Revenue</p>
          <p className="text-2xl font-bold">${metrics.adRevenue.toFixed(2)}</p>
        </div>
      </div>
        ...
        <form action="/api/admin/frequency" method="post" className="flex flex-wrap items-end gap-3">
          ...
          <label className="text-sm">
            ...
            <select name="trendWindow" defaultValue={settings.trendWindow} className="ml-2 rounded border px-2 py-1">
              ...
              <option value="now 1-d">Today</option>
            </select>
          </label>
      ...
      <div className="rounded-xl border bg-white p-4 dark:bg-zinc-900">
        <h2 className="mb-3 text-xl font-semibold">Approve / Edit / Delete Posts</h2>
          ...
          {articles.map((article) => (
            <div key={article.id} className="flex items-center justify-between rounded-lg border p-3">
              ...
              <form action={`/api/articles/${article.id}`} method="post" className="flex gap-2 text-sm">
                ...
                <button name="_action" value="delete" className="rounded border px-2 py-1 text-red-600">
                  Delete
                </button>
              </form>
            </div>
